...
import { ArrowRightLeft, Users, X, Check, CheckCircle } from "lucide-react";
import moment from "moment";

export default function ScambiTurnoView({
  scambiDaMePending,
  scambiPerMe,
  turniFuturi,
  currentUser,
  ...
  return (
    <div className="space-y-4">
      ...
      <NeumorphicCard className="p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Richieste di scambio che hai inviato ai tuoi colleghi</p>
                    ...
                    <div className="flex flex-col gap-2 ml-3">
                      {statoScambio === 'pending' ? (
                        <>
                          ...
                          <button onClick={() => cancellaScambioMutation.mutate({ mioTurnoId: mioTurno.id, suoTurnoId })} disabled={cancellaScambioMutation.isPending} className="px-3 py-1.5 bg-red-500 text-white rounded-lg text-xs font-medium hover:bg-red-600 flex items-center gap-1">
                            <X className="w-3 h-3" /> Cancella
                          </button>
                        </>
                      ) : (
                        <div className="px-3 py-2 bg-blue-100 text-blue-700 rounded-lg text-xs font-medium text-center">
                          <CheckCircle className="w-4 h-4 mx-auto mb-1" />Accettato<br /><span className="text-[10px]">In attesa manager</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <NeumorphicCard className="p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Colleghi che vogliono scambiare il loro turno con te</p>
                ...
                <div key={turno.id} className="neumorphic-pressed p-4 rounded-xl">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex flex-col gap-2 ml-3">
                      {statoScambio === 'pending' ? (
                        <>
                          <button onClick={() => rispondiScambioMutation.mutate({ suoTurnoId: turno.id, accetta: true })} disabled={rispondiScambioMutation.isPending} className="px-3 py-1.5 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 flex items-center gap-1">
                            <Check className="w-3 h-3" /> Accetta
                          </button>
                          <button onClick={() => rispondiScambioMutation.mutate({ suoTurnoId: turno.id, accetta: false })} disabled={rispondiScambioMutation.isPending} className="px-3 py-1.5 bg-red-500 text-white rounded-lg text-sm font-medium hover:bg-red-600 flex items-center gap-1">
                            <X className="w-3 h-3" /> Rifiuta
                          </button>
                        </>
                      ) : (
                        <div className="px-3 py-2 bg-blue-100 text-blue-700 rounded-lg text-xs font-medium text-center">
                          <CheckCircle className="w-4 h-4 mx-auto mb-1" />Accettato<br /><span className="text-[10px]">In attesa approvazione Store Manager</span>
                        </div>
                      )}
                    </div>
                  </div>
