...
import { TrendingUp, MousePointerClick, Eye, DollarSign, Target, RefreshCw, Calendar } from 'lucide-react';
...
export default function Google() {
  ...
  const syncMutation = useMutation({
    mutationFn: async () => {
      const response = await base44.functions.invoke('syncGoogleAds', {});
  ...
  const stats = useMemo(() => {
    return {
      totalCost: filteredCampaigns.reduce((sum, c) => sum + (c.cost || 0), 0),
  ...
  const chartData = useMemo(() => {
    ...
    filteredCampaigns.forEach(c => {
      ...
      byDate[dateKey].cost += c.cost || 0;
      byDate[dateKey].clicks += c.clicks || 0;
      byDate[dateKey].conversions += c.conversions || 0;
      byDate[dateKey].conversionValue += c.conversion_value || 0;
    });
  ...
  const campaignPerformance = useMemo(() => {
    ...
    filteredCampaigns.forEach(c => {
      ...
      byCampaign[c.campaign_id].cost += c.cost || 0;
  ...
  return (
    <ProtectedPage pageName="Google" requiredUserTypes={['admin', 'manager']}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <NeumorphicCard className="p-6 text-center">
            ...
            <h3 className="text-2xl font-bold text-[#6b6b6b] mb-1">€{stats.totalCost.toFixed(2)}</h3>
          ...
          <NeumorphicCard className="p-6 text-center">
            ...
            <h3 className="text-2xl font-bold text-[#6b6b6b] mb-1">{filteredCampaigns.length}</h3>
            <p className="text-sm text-[#9b9b9b]">Record</p>
          </NeumorphicCard>
        </div>
