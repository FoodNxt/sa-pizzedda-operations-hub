...
import UnpaidAbsenceModal from "../components/payroll/UnpaidAbsenceModal";
...
export default function Payroll() {
  ...
  const shifts = useMemo(() => {
    return turniPlanday.map((turno) => {
      ...
      if (turno.ora_inizio && turno.ora_fine) {
        ...
        // Turno a cavallo della mezzanotte (es. 19:00 → 01:00): aggiungi 24h
  ...
  const exportAllEmployeesDailyNewFormat = (downloadFmt, includeOvertime) => {
    ...
    // Get all weeks and build columns structure
    const weeks = new Map();
    ...
    // Build header row with days and week recap columns interleaved
    csv += 'Dipendente,';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
          ...
          <NeumorphicCard className="max-w-6xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-start justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                onClick={() => setShowDownloadModal('employee')}
                className="neumorphic-flat px-4 py-2 rounded-lg flex items-center gap-2 text-[#6b6b6b] hover:text-[#8b7355] transition-colors">
      ...
      {/* ✅ Unpaid Absence Detail Modal */}
      {showUnpaidAbsenceModal && unpaidAbsenceDetails &&
      <UnpaidAbsenceModal
        details={unpaidAbsenceDetails}
        startDate={startDate}
        endDate={endDate}
        minutesToHours={minutesToHours}
        onClose={() => setShowUnpaidAbsenceModal(false)} />
      }
