  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {activeTab === 'analytics' && <><FilterBar />
        ...
        <NeumorphicCard className="p-6 mb-6"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-[#6b6b6b]">Trend</h3><div className="flex gap-2 flex-wrap"><div className="flex gap-1 neumorphic-pressed rounded-lg p-1"><button onClick={() => { if (showCount && !showRefunds) return; setShowCount(!showCount); }} className={`px-3 py-1 rounded-lg text-xs font-medium ${showCount ? 'bg-[#8b7355] text-white' : 'text-[#6b6b6b]'}`}>Ordini</button><button onClick={() => { if (showRefunds && !showCount) return; setShowRefunds(!showRefunds); }} className={`px-3 py-1 rounded-lg text-xs font-medium ${showRefunds ? 'bg-red-600 text-white' : 'text-[#6b6b6b]'}`}>€ Rimborsi</button></div><div className="flex gap-1 neumorphic-pressed rounded-lg p-1">{['daily','weekly','monthly'].map((v) => <button key={v} onClick={() => setTrendView(v)} className={`px-3 py-1 rounded-lg text-xs font-medium ${trendView === v ? 'bg-blue-500 text-white' : 'text-[#6b6b6b]'}`}>{v === 'daily' ? 'Giorno' : v === 'weekly' ? 'Settimana' : 'Mese'}</button>)}</div><NeumorphicButton onClick={() => analyzeWithAI('byDate')} disabled={!analyticsData.byDate.length || loadingAI} className="text-sm"><Sparkles className="w-4 h-4" /></NeumorphicButton></div></div>{analyticsData.byDate.length > 0 ? <ResponsiveContainer width="100%" height={300}><LineChart data={analyticsData.byDate}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Legend />{showCount && <Line type="monotone" dataKey="count" stroke="#8b7355" name="Ordini" strokeWidth={2} />}{showRefunds && <Line type="monotone" dataKey="refunds" stroke="#dc2626" name="€ Rimborsi" strokeWidth={2} />}</LineChart></ResponsiveContainer> : <p className="text-center text-[#9b9b9b] py-8">Nessun dato</p>}</NeumorphicCard>
      </>}
