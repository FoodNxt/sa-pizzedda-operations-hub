            ...
            <NeumorphicCard className="p-6">
              ...
              stores.forEach((store) => {
                ...
                const utenzeStore = utenze.filter((u) => {
                  if (u.assegnazione === 'singolo') return u.store_id === storeId;else
                  if (u.assegnazione === 'multipli') return u.stores_ids?.includes(storeId);
                  return false;
                });
                ...
                reduce((sum, s) => {
                  if (s.assegnazione === 'singolo' && s.store_id === storeId) return sum + (s.costo || 0);else
                  if (s.assegnazione === 'multipli' && s.stores_ids?.includes(storeId)) return sum + (s.costo || 0);else
                  if (s.assegnazione === 'tutti') return sum + (s.costo || 0) / numStores;
                  return sum;
                }, 0) * proRata;
                ...
                reduce((sum, s) => {
                  if (s.assegnazione === 'singolo' && s.store_id === storeId) return sum + (s.costo || 0) / 12;else
                  if (s.assegnazione === 'multipli' && s.stores_ids?.includes(storeId)) return sum + (s.costo || 0) / 12;else
                  if (s.assegnazione === 'tutti') return sum + (s.costo || 0) / 12 / numStores;
                  return sum;
                }, 0) * proRata;
                ...
                const costoAds = budgetAds.reduce((sum, b) => {
                  if (b.assegnazione === 'singolo' && b.store_id === storeId) return sum + (b.budget_mensile || 0);else
                  if (b.assegnazione === 'tutti') return sum + (b.budget_mensile || 0) / numStores;
                  return sum;
                }, 0) * proRata;
