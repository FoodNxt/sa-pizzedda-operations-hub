          ...
          <>
            <NeumorphicCard className="p-6">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-800">Checklist Dipendenti</h2>
                <NeumorphicButton
            ...
            {showChecklistForm && (
              <NeumorphicCard className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={resetChecklistForm} className="nav-button p-2 rounded-lg">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={checklistForm.titolo}
                      onChange={(e) => setChecklistForm({ ...checklistForm, titolo: e.target.value })}
                      className="w-full neumorphic-pressed px-4 py-3 rounded-xl text-slate-700 outline-none"
                      placeholder="Es: Apertura Mattina"
                    />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-slate-700 mb-1 block">Categoria *</label>
                    <select
                      ...
                      onChange={(e) => setChecklistForm({ ...checklistForm, categoria: e.target.value })}
                      className="w-full neumorphic-pressed px-4 py-3 rounded-xl text-slate-700 outline-none"
                    >
                      {CHECKLIST_CATEGORIE.map(cat => (
                        <option key={cat} value={cat}>{cat}</option>
                      ))}
                    </select>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    value={checklistForm.descrizione}
                    onChange={(e) => setChecklistForm({ ...checklistForm, descrizione: e.target.value })}
                    className="w-full neumorphic-pressed px-4 py-3 rounded-xl text-slate-700 outline-none min-h-[80px]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {RUOLI.map(ruolo => (
                        <button
                          key={ruolo}
                          type="button"
                          onClick={() => toggleRuolo(ruolo)}
                          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                            (checklistForm.ruoli_assegnati || []).includes(ruolo)
                  ...
                  <div>
                    <label className="text-sm font-medium text-slate-700 mb-1 block">Store Specifico</label>
                    <select
                      value={checklistForm.store_specifico}
                      onChange={(e) => setChecklistForm({ ...checklistForm, store_specifico: e.target.value })}
                      className="w-full neumorphic-pressed px-4 py-3 rounded-xl text-slate-700 outline-none"
                    >
                      <option value="">Tutti i locali</option>
                      {stores.map(store => (
                ...
                <div className="mb-4">
                  <label className="text-sm font-medium text-slate-700 mb-2 block">
                    Elementi Checklist ({checklistForm.items.length})
                  </label>
                  
                  {checklistForm.items.length > 0 && (
                    <div className="space-y-2 mb-3">
                      {checklistForm.items.map((item, idx) => (
                        <div key={item.id} className="neumorphic-pressed p-3 rounded-lg flex items-center gap-3">
                          <GripVertical className="w-4 h-4 text-slate-400" />
                          <span className="text-sm text-slate-500">{idx + 1}.</span>
                          <span className="flex-1 text-slate-700">{item.testo}</span>
                          <button
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <NeumorphicButton onClick={addChecklistItem}>
                      <Plus className="w-4 h-4" />
                    </NeumorphicButton>
                  </div>
                </div>
                ...
                <div className="flex items-center mb-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={checklistForm.attivo}
                      onChange={(e) => setChecklistForm({ ...checklistForm, attivo: e.target.checked })}
                      className="w-5 h-5"
                    />
                ...
                <div className="flex gap-3">
                  <NeumorphicButton onClick={resetChecklistForm} className="flex-1">Annulla</NeumorphicButton>
                  <NeumorphicButton 
                    onClick={handleSaveChecklist} 
                    variant="primary" 
                    className="flex-1 flex items-center justify-center gap-2"
                    disabled={!checklistForm.titolo || checklistForm.items.length === 0}
                  >
                    <Save className="w-4 h-4" />
                    Salva
                  </NeumorphicButton>
            ...
            {/* Lista Checklist */}
            <div className="space-y-4">
              {CHECKLIST_CATEGORIE.map(categoria => {
                const categoryChecklists = checklists.filter(c => c.categoria === categoria);
                ...
                return (
                  <NeumorphicCard key={categoria} className="p-4">
                    <h3 className="font-bold text-slate-800 mb-3 flex items-center gap-2">
                      ...
                      {categoria}
                      <span className="text-sm font-normal text-slate-500">({categoryChecklists.length})</span>
                    </h3>
                    <div className="space-y-2">
                      {categoryChecklists.map(checklist => (
                        <div 
                          key={checklist.id} 
                          className={`neumorphic-pressed p-4 rounded-xl ${!checklist.attivo ? 'opacity-50' : ''}`}
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <h4 className="font-medium text-slate-800">{checklist.titolo}</h4>
                              {checklist.descrizione && (
                                <p className="text-sm text-slate-600 mt-1">{checklist.descrizione}</p>
                              )}
                              <div className="flex flex-wrap gap-2 mt-2">
                                <span className="px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-xs">
                                  {checklist.items?.length || 0} attività
                                </span>
                                {(checklist.ruoli_assegnati || []).map(ruolo => (
                                  <span key={ruolo} className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs">
                                    {ruolo}
                                  </span>
                                ))}
                                {checklist.store_specifico && (
                                  <span className="px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 text-xs flex items-center gap-1">
                                    <Store className="w-3 h-3" />
                                    {getStoreName(checklist.store_specifico)}
                                  </span>
                                )}
                              </div>
                                ...
                                <div className="mt-3 space-y-1">
                                  ...
                                  {checklist.items.length > 3 && (
                                    <p className="text-xs text-slate-400 ml-5">
                                      + altre {checklist.items.length - 3} attività
                                    </p>
                                  )}
                                </div>
                              )}
