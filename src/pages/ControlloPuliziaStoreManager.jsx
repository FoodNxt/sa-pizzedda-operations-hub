...
export default function ControlloPuliziaStoreManager() {
  ...
  // Load domande for Store Manager - solo dopo selezione store
  const { data: domande = [], isLoading: isLoadingDomande } = useQuery({
    queryKey: ['domande-pulizia-storemanager', selectedStore],
    queryFn: async () => {
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-200">
                          ...
                          {previews[`${domanda.id}_foto`] ? (
                            ...
                            <div>
                              <button
                                type="button"
                                onClick={() => document.getElementById(`photo-${domanda.id}_foto`).click()}
                                className="w-full neumorphic-flat p-6 rounded-lg text-center"
                                disabled={uploading}
                              >
                                <Upload className="w-6 h-6 text-[#9b9b9b] mx-auto mb-2" />
                                <p className="text-sm text-[#9b9b9b]">Carica Foto</p>
                              </button>
                              <input
                                id={`photo-${domanda.id}_foto`}
                                type="file"
                                accept="image/*"
                                capture="environment"
                                onChange={(e) => handlePhotoChange(`${domanda.id}_foto`, e)}
                                className="hidden"
                                disabled={uploading}
                              />
                            </div>
