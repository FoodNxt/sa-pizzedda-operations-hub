...
import RevenueTeglia from "../components/precotture/RevenueTeglia";
import DeltaTeglie from "../components/precotture/DeltaTeglie";
import { ChefHat, Plus, Edit, Save, X, Calendar, History, Store, User, Trash2, Pizza, TrendingUp, Settings, CheckCircle2 as CheckCircle, AlertTriangle, ChevronDown } from "lucide-react";
    ...
    <ProtectedPage pageName="PrecottureAdmin" requiredUserTypes={['admin']}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={() => setActiveTab('revenue-teglia')}
            className={`px-6 py-3 rounded-xl font-medium transition-all flex items-center gap-2 ${
            activeTab === 'revenue-teglia' ?
            'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg' :
            'neumorphic-flat text-slate-700'}`
            }>
        ...
        {activeTab === 'delta-teglie' && (
          <DeltaTeglie
            deltaStartDate={deltaStartDate}
            setDeltaStartDate={setDeltaStartDate}
            deltaEndDate={deltaEndDate}
            setDeltaEndDate={setDeltaEndDate}
            selectedStore={selectedStore}
            precottureForm={precottureForm}
            prodottiVendutiDelta={prodottiVendutiDelta}
            teglieButtate={teglieButtate}
            teglieConfig={teglieConfig}
          />
        )}
