          ...
          <div className="flex items-center gap-3">
            {importLogs.length > 0 && (
              <div className="text-right bg-slate-50 px-4 py-2 rounded-lg border border-slate-200">
                <p className="text-xs text-slate-500 font-medium">Ultima importazione automatica</p>
                <p className="text-base font-bold text-slate-800">
                  {format(parseISO(importLogs[0].timestamp), 'dd/MM/yy HH:mm', { locale: it })}
                </p>
                {importLogs[0].status === 'success' && (
                  <p className="text-xs text-green-600 font-medium">✓ {importLogs[0].imported_count} record importati</p>
                )}
                {importLogs[0].status === 'error' && (
                  <p className="text-xs text-red-600 font-medium">✗ Errore nell'importazione</p>
                )}
              </div>
            )}
            {importLogs.length === 0 && (
              <div className="text-right bg-amber-50 px-4 py-2 rounded-lg border border-amber-200">
                <p className="text-xs text-amber-700 font-medium">Nessuna importazione automatica ancora effettuata</p>
                <p className="text-xs text-amber-600">Prossima esecuzione tra ~1 ora</p>
              </div>
            )}
            <NeumorphicButton 
              onClick={async () => {
