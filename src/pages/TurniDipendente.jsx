...
import TurniLiberiView from "../components/dipendente/TurniLiberiView";
import DisponibilitaView from "../components/dipendente/DisponibilitaView";
import ScambioModal from "../components/dipendente/ScambioModal";
import {
...
export default function TurniDipendente() {
  ...
  const { data: tuttiTurniGiornoScambio = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await base44.functions.invoke('getAllDipendentiForPlanday', {
        filter_data: selectedTurnoScambio.data
      });
  ...
  return (
    <ProtectedPage pageName="TurniDipendente">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* VISTA: SCAMBI */}
        {activeView === 'scambi' && (
          <ScambiTurnoView scambiDaMePending={scambiDaMePending} scambiPerMe={scambiPerMe} turniFuturi={turniFuturi} currentUser={currentUser} getStoreName={getStoreName} cancellaScambioMutation={cancellaScambioMutation} rispondiScambioMutation={rispondiScambioMutation} />
        )}

        {activeView === 'liberi' && <TurniLiberiView turniLiberi={turniLiberi} currentUser={currentUser} mieRichiesteTurni={mieRichiesteTurni} richiediTurnoLiberoMutation={richiediTurnoLiberoMutation} getStoreName={getStoreName} />}

        {activeView === 'disponibilita' && currentUser && <DisponibilitaView currentUser={currentUser} mieDisponibilita={mieDisponibilita} setActiveView={setActiveView} />}

        <ScambioModal show={showScambioModal} selectedTurnoScambio={selectedTurnoScambio} colleghiPerScambio={colleghiPerScambio} selectedCollegaScambio={selectedCollegaScambio} setSelectedCollegaScambio={setSelectedCollegaScambio} selectedTurnoCollegaScambio={selectedTurnoCollegaScambio} setSelectedTurnoCollegaScambio={setSelectedTurnoCollegaScambio} turniFuturiCollega={turniFuturiCollega} allUsersData={allUsersData} richiestaScambioMutation={richiestaScambioMutation} getStoreName={getStoreName} onClose={() => { setShowScambioModal(false); setSelectedTurnoScambio(null); setSelectedCollegaScambio(null); setSelectedTurnoCollegaScambio(null); }} />
      </div>
    </ProtectedPage>);
}